...
install -m 0755 "$ROOT/service.sh" "$PREFIX/bin/multica-core-service"
install -m 0755 "$ROOT/uninstall.sh" "$PREFIX/bin/multica-core-uninstall"
rm -rf "$SHARE_DIR/web"
...
printf 'Service:   %s/bin/multica-core-service start --data-dir %s\n' "$PREFIX" "$DATA_DIR"
printf 'Uninstall: %s/bin/multica-core-uninstall --data-dir %s\n' "$PREFIX" "$DATA_DIR"
